            ...
            @foreach($employee_status as $item)
                <tr>
                    ...
                    <td class="border border-slate-500 px-3">
                        ...
                        @include('admin.components.table_update_form', ['title'=>'社員ステータス', 'id'=>$item->id, 'name'=>$item->name, 'role'=>\Illuminate\Support\Facades\Session::get('admin.role')])
                    </td>
                    <td class="border border-slate-500 px-3">
                        <!--
                            todo:削除ボタン
                        ...
                        @include('admin.components.table_delete_form', ['title'=>'社員ステータス', 'id'=>$item->id, 'role'=>\Illuminate\Support\Facades\Session::get('admin.role')])
                    </td>
                </tr>
            @endforeach
            </tbody>
