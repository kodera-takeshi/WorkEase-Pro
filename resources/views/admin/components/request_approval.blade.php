...
<a
    href="#examination-{{ $id }}"
    class="block rounded-lg rounded-full bg-blue-500 text-white py-2 px-4 w-3/5 my-2 mx-auto mx-auto text-center font-bold"
>
    申請承認
</a>


<div id="examination-{{ $id }}" class="hidden target:block">
    <div class="block w-full h-full bg-black/70 absolute top-0 left-0">
        ...
        <div class="w-1/5 mx-auto mt-20 relative -top-full bg-white p-5 rounded-lg">
            ...
            <form action="{{ route('admin.requests.approval') }}" method="POST">
                @csrf
                <label
                    for="id"
                ...
                <div class="mt-10">
                    <button
                        type="submit"
                        class="block rounded-md bg-green-600 px-3.5 py-2.5 text-center text-sm font-semibold text-white shadow-sm hover:bg-green-500 focus-visible:outline focus-visible:outline-2 focus-visible:green-offset-2 focus-visible:outline-green-600 mx-auto"
                    >
                        承認
                    </button>
                </div>
